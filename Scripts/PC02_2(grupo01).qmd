...
Las dos hipotesis son nulas ya que los valores de P son menores a 0.05 y nunguna tiene una distribución normal
...
```{r}
var.test(PSA_preoperatorio ~ Raza_afromericana, data = almac_sangre)
```
...
```{r}
t.test(PSA_preoperatori ~ Raza_afroamericana, data = almac_sangre, var.equal = TRUE)
```
